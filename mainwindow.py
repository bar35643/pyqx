...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def createPopupMenu(self):
        ...
        # Reimplementando esta función conseguimos que no se creen los menús popup cuando hacemos click derecho en toolbars/dockwidgets.
        pass
    ...
    def createToolBarActions(self):
        ...
        tools = ["selection", "magicwand", "pencil", "eraser",
                 "colorpicker", "fill", "gradient", "exchange"]
        connects = [lambda: self.context.changeCurrentTool(Pixeler.Tools.Selection),
                    lambda: self.context.changeCurrentTool(
                        Pixeler.Tools.MagicWand),
                    lambda: self.context.changeCurrentTool(
                        Pixeler.Tools.Pencil),
                    lambda: self.context.changeCurrentTool(
                        Pixeler.Tools.Eraser),
                    lambda: self.context.changeCurrentTool(
                        Pixeler.Tools.ColorPicker),
                    lambda: self.context.changeCurrentTool(Pixeler.Tools.Fill),
                    lambda: self.context.changeCurrentTool(
                        Pixeler.Tools.Gradient),
                    lambda: self.context.changeCurrentTool(Pixeler.Tools.Exchange)]
        shortcuts = ['Z', '', 'X', 'C', 'A', 'S', 'D', '']

        for i in range(len(tools)):
            a = QtWidgets.QAction(QtGui.QIcon(os.path.join("themes", self.context.theme,
                                  tools[i] + ".png")), self.context.getText("tools", tools[i]) + " (" + shortcuts[i] + ")", self.tools)
            a.setCheckable(True)
        ...
        a = QtWidgets.QAction(QtGui.QIcon(os.path.join(
            "themes", self.context.theme, "zoomin.png")), self.context.getText("tools", "zoomin"), self.tools)
        a.setShortcut("Ctrl++")
        a.triggered.connect(self.zoomIn)
        l.append(a)

        a = QtWidgets.QAction(QtGui.QIcon(os.path.join(
            "themes", self.context.theme, "zoomout.png")), self.context.getText("tools", "zoomout"), self.tools)
        a.setShortcut("Ctrl+-")
    ...
    def createFileActions(self):
        ...
        icons = ["document-new.png", "document-open.png",
                 "document-save.png", "document-save-as.png", "application-exit.png"]
        shortcuts = ['Ctrl+N', 'Ctrl+O', 'Ctrl+S', 'Ctrl+Shift+S', 'Ctrl+Q']
        connects = [self.newFile, self.openFile,
                    self.saveFile, self.saveFileAs, self.close]

        l = []

        for i in range(len(ids)):
            a = QtWidgets.QAction(QtGui.QIcon(
                "images/" + icons[i]), self.context.getText("menu_file_labels", ids[i]), self)
            a.setShortcut(shortcuts[i])
            a.triggered.connect(self.restoreFocus)
            a.setStatusTip(self.context.getText(
                "menu_file_status_tips", ids[i]))
            if connects[i] != 0:
    ...
    def createEditActions(self):
        ...
        ids = ["undo", "redo", "selectall", "deselect",
               "cut", "copy", "paste", "clear", "preferences"]
        icons = ["edit-undo.png", "edit-redo.png", "", "", "edit-cut.png",
                 "edit-copy.png", "edit-paste.png", "edit-clear.png", "document-properties.png"]
        shortcuts = ['Ctrl+Z', 'Ctrl+Y', "Ctrl+A", "Ctrl+Shift+A",
                     'Ctrl+X', 'Ctrl+C', 'Ctrl+V', 'Del', '']
        connects = [self.undo, self.redo, self.selectAll, self.deselect,
                    self.cut, self.copy, self.paste, self.clear, self.showPreferences]

        l = []

        for i in range(len(ids)):
            a = QtWidgets.QAction(QtGui.QIcon(
                "images/" + icons[i]), self.context.getText("menu_edit_labels", ids[i]), self)
            a.setShortcut(shortcuts[i])
            a.triggered.connect(self.restoreFocus)
            a.setStatusTip(self.context.getText(
                "menu_edit_status_tips", ids[i]))
            if connects[i] != 0:
                a.triggered.connect(connects[i])
            l.append(a)
    ...
    def createViewActions(self):
        ...
        for i in range(len(ids)):
            a = QtWidgets.QAction(QtGui.QIcon(
                "images/" + icons[i]), self.context.getText("menu_view_labels", ids[i]), self)
            a.setShortcut(shortcuts[i])
            a.triggered.connect(self.restoreFocus)
            a.setStatusTip(self.context.getText(
                "menu_view_status_tips", ids[i]))
            if connects[i] != 0:
                a.triggered.connect(connects[i])
            a.setCheckable(True)
            l.append(a)
    ...
    def createTransformActions(self):
        ...
        ids = ["flip_hor", "flip_ver", "rotate_cw", "rotate_ccw",
               "rotate_180", "resize", "resize_canvas"]
        icons = ["", "", "", "", "", "", ""]
        shortcuts = ['', '', '', '', '', '', '']
        connects = [self.flipHorizontally, self.flipVertically, self.rotate90CW, self.rotate90CCW,
                    self.rotate180, self.showResizeImageDialog, self.showResizeCanvasDialog]

        l = []

        for i in range(len(ids)):
            a = QtWidgets.QAction(QtGui.QIcon(
                "images/" + icons[i]), self.context.getText("menu_transform_labels", ids[i]), self)
            a.setShortcut(shortcuts[i])
            a.triggered.connect(self.restoreFocus)
            a.setStatusTip(self.context.getText(
                "menu_transform_status_tips", ids[i]))
            if connects[i] != 0:
                a.triggered.connect(connects[i])
            l.append(a)
    ...
    def createHelpActions(self):
        ...
        for i in range(len(ids)):
            a = QtWidgets.QAction(QtGui.QIcon(
                "images/" + icons[i]), self.context.getText("menu_help_labels", ids[i]), self)
            a.setShortcut(shortcuts[i])
            a.triggered.connect(self.restoreFocus)
            a.setStatusTip(self.context.getText(
                "menu_help_status_tips", ids[i]))
            if connects[i] != 0:
                a.triggered.connect(connects[i])
            l.append(a)
    ...
    def createDockWidgets(self):
        ...
        self.toolProperties = ToolProperties(self.context.getText(
            "dock_widgets", "tool_properties"), self.context, self.signals)
        ...
        # Preview
        self.preview = Preview(self.context.getText(
            "dock_widgets", "preview"), self.context, self.signals, self)
    ...
    def undo(self):
        ...
        if self.context.currentImage().posHistory > 0:
            ...
            self.context.currentImage().image = QtGui.QImage(
                self.context.currentImage().history[self.context.currentImage().posHistory])
    ...
    def flipHorizontally(self):
        ...
        self.context.currentImage().image = image.transformed(
            QtGui.QTransform().rotate(180.0)).mirrored()
    ...
    def rotate90CCW(self):
        image = self.context.currentImage().image
        self.context.currentImage().image = image.transformed(
            QtGui.QTransform().rotate(270.0))

    def rotate180(self):
        image = self.context.currentImage().image
        self.context.currentImage().image = image.transformed(
            QtGui.QTransform().rotate(180.0))
