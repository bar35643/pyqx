...
class ScrollArea(QtWidgets.QScrollArea):
    ...
    def calcNewCanvasPosition(self):
        ...
        if self.context.imagePos == self.index:
            ...
            if self.canvas.width() < w:
                self.canvas.move((w - self.context.getImagePos(self.index).image.width()
                                 * self.context.getImagePos(self.index).zoom) / 2, self.canvas.y())
            if self.canvas.height() < h:
                self.canvas.move(self.canvas.x(), (h - self.context.getImagePos(
                    self.index).image.height() * self.context.getImagePos(self.index).zoom) / 2)

    def calcNewScrollBarPosition(self):

        if self.context.imagePos == self.index:
            self.horizontalScrollBar().setValue((self.horizontalScrollBar().maximum() -
                                                 self.horizontalScrollBar().minimum()) / 2)
    ...
    def setNewImageZoom(self):
        ...
        self.context.images[-1].zoom = min(w / self.context.images[-1].image.width(
